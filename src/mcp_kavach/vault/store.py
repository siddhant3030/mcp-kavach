...
``$KAVACH_DATA_DIR/vault.db``, mode 0600. Lookups never store plaintext
...
``cryptography`` package — installed via ``mcp-kavach[vault]``). Fernet
...
from mcp_kavach.audit import hmac_value
    ...
    def __init__(self, path: str | Path | None = None, *, scope: str = DEFAULT_SCOPE) -> None:
        try:
            ...
            raise VaultError(
                ...
                "install with: pip install 'mcp-kavach[vault]'"
            ) from err
        ...
        self._salt = hashlib.sha256(b"mcp-kavach.vault.hmac-salt:" + key).digest()
