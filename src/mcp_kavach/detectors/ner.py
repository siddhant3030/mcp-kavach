...
from mcp_kavach.detectors.base import Detector
...
# Presidio entity name -> kavach entity name (the policy vocabulary).
PRESIDIO_TO_KAVACH: dict[str, str] = {
    "PERSON": "PERSON_NAME",
    "LOCATION": "ADDRESS",
...
NER_ENTITY_TYPES = frozenset(PRESIDIO_TO_KAVACH.values())
...
class NerDetector(Detector):
    ...
    def __init__(self, analyzer: object, entities: frozenset[str]) -> None:
        ...
        self._presidio_entities = sorted(
            p for p, k in PRESIDIO_TO_KAVACH.items() if k in entities
        )
    ...
    def detect(self, text: str) -> list[Span]:
        ...
        spans: list[Span] = []
        for r in results:
            entity = PRESIDIO_TO_KAVACH.get(r.entity_type)
            if entity is None:
                continue
            spans.append(
                Span(
...
def load_ner_detector(entities: frozenset[str]) -> NerDetector | None:
    """Build a detector for the given kavach entity names, reusing the
