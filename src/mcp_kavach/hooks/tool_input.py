...
def handle(data: dict) -> dict | None:
    ...
    if result.blocked:
        return _decision("deny", f"kavach: {result.block_reason}")
    ...
    if (
        ...
        return _decision("allow", f"kavach: masked {entities} in the tool input", result.payload)
    if cfg.tool_input_guard in ("ask", "mask"):
        return _decision(
            "ask",
            f"kavach: this call to '{tool}' contains {entities} — share it with the tool anyway?",
        )
    return {
        "systemMessage": f"kavach: tool '{tool}' was called with {entities} in its input."
    }
