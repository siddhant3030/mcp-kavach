...
def replacement_text(cluster: Cluster, original: str, policy_name: str) -> str:
...
def transform_text(text: str, findings: list[Finding], policy_name: str) -> str:
    ...
    for cluster in sorted(merge_clusters(findings), key=lambda c: c.start, reverse=True):
        rep = replacement_text(cluster, text, policy_name)
        text = text[: cluster.start] + rep + text[cluster.end :]
    return text


def transform_whole_value(value: object, findings: list[Finding], policy_name: str) -> object:
    ...
    return replacement_text(cluster, str(value), policy_name)
