"""Hook configuration. Precedence: env vars > ~/.virelia/config.yaml > defaults."""
...
from virelia.audit import AuditSink, open_sink
from virelia.engine import Engine
from virelia.hooks.runner import data_dir, hmac_salt
from virelia.policy import Policy, load_policy, load_preset
...
class HookConfig:
    ...
    # postgres:// URL. Empty means $VIRELIA_DATA_DIR/audit.jsonl.
    audit: str = ""


def load_config() -> HookConfig:
    cfg = HookConfig()
    path = Path(os.environ.get("VIRELIA_CONFIG") or Path.home() / ".virelia" / "config.yaml")
    if path.is_file():
    ...
    env = os.environ
    cfg.policy = env.get("VIRELIA_POLICY", cfg.policy)
    cfg.prompt_guard = env.get("VIRELIA_PROMPT_MODE", cfg.prompt_guard)
    cfg.tool_input_guard = env.get("VIRELIA_TOOL_INPUT_MODE", cfg.tool_input_guard)
    cfg.tool_output_guard = env.get("VIRELIA_TOOL_OUTPUT_MODE", cfg.tool_output_guard)
    if env.get("VIRELIA_TOOL_ALLOWLIST"):
        cfg.tool_allowlist = tuple(
            t.strip() for t in env["VIRELIA_TOOL_ALLOWLIST"].split(",") if t.strip()
        )
    if env.get("VIRELIA_CONFIRM_WINDOW"):
        cfg.confirm_window_seconds = int(env["VIRELIA_CONFIRM_WINDOW"])
    cfg.audit = env.get("VIRELIA_AUDIT", cfg.audit)
    return cfg
...
def audit_sink(cfg: HookConfig) -> AuditSink:
    ...
    `audit` config key or VIRELIA_AUDIT)."""
