...
from virelia.hooks import state
...
def handle(data: dict) -> dict | None:
    ...
    if cfg.prompt_guard == "warn":
        return {
            "systemMessage": f"virelia: your prompt contained {entities} and was sent as-is "
    ...
    if state.consume(session, digest, cfg.confirm_window_seconds):
        return {"systemMessage": "virelia: PII confirmed by resend — prompt sent unmasked."}
    ...
    message = (
        f"virelia blocked this prompt — it contains {entities}.\n\n"
