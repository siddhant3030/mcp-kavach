...
arguments) through a virelia Engine.

Requires the [proxy] extra. The module imports without fastmcp installed;
accessing ``VireliaMiddleware`` lazily builds the class (PEP 562) and raises
...
def _build_middleware_class() -> type:
    try:
        ...
        raise ImportError(
            "VireliaMiddleware requires fastmcp — install with: "
            "pip install 'virelia[proxy]'"
        ) from err

    class VireliaMiddleware(Middleware):
        ...
        async def on_call_tool(self, context, call_next):
            ...
            if self.scan_arguments and context.message.arguments:
                ...
                if request.blocked:
                    raise ToolError(request.block_reason or "blocked by virelia")
                if request.modified:
            ...
            try:
                ...
                return ToolResult(
                    content=[
                        TextContent(
                            type="text",
                            text="virelia failed to scan this result and is "
        ...
        def _scrub(self, tool: str, result):
            ...
            if blocked_payload is not None:
                if self.block_mode == "error":
                    raise ToolError(
                        str(blocked_payload.get("message", "blocked by virelia"))
                    )
                return ToolResult(
                    content=[
