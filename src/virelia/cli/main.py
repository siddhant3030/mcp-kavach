"""virelia CLI: hook handlers, dry-run scanning, and the masking MCP proxy."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    proxy_p.add_argument("--name", default="virelia-proxy")
    proxy_p.add_argument(
        "--vault", nargs="?", const="", metavar="PATH",
        help="enable tokenize actions; optional vault path (default: "
        "$VIRELIA_DATA_DIR/vault.db)",
    ...
    rehydrate_p.add_argument(
        "--vault", metavar="PATH",
        help="vault path (default: $VIRELIA_DATA_DIR/vault.db)",
    )
    ...
    from virelia import __version__

    print(f"virelia {__version__}")
    return 0


def _hook(event: str) -> int:
    from virelia.hooks import prompt_guard, tool_input, tool_output
    from virelia.hooks.runner import run_hook
...
def _open_vault(args: argparse.Namespace):
    ...
    if getattr(args, "vault", None) is None:
        return None
    from virelia.vault import DEFAULT_SCOPE, Vault
...
def _scan(args: argparse.Namespace) -> int:
    ...
    from virelia.hooks.config import resolve_policy

    engine = Engine(
        resolve_policy(args.policy), hmac_salt=b"virelia-scan-dry-run", vault=_open_vault(args)
    )
...
def _audit(args: argparse.Namespace) -> int:
    from virelia.cli.audit import run_report, run_tail
...
def _proxy(args: argparse.Namespace) -> int:
    ...
    from virelia.audit import open_sink
    from virelia.hooks.config import resolve_policy
    from virelia.hooks.runner import hmac_salt
